...
//随机文字烟花内容
const randomWords = ["新春快乐", "马年大吉", "马上有钱", "马到成功", "Oscar Studio"];
const wordDotsMap = {};
...
window.addEventListener("message", (event) => {
	if (event.data === "start-countdown") {
		startNewYearCountdown();
	} else if (event.data === "start-finale") {
		startNewYearFinale();
	}
});
...
function startNewYearCountdown() {
	...
	const interval = setInterval(() => {
		...
		// Launch a text firework for the current number
		const centerX = 0.5;
		const centerY = 0.5;
		const color = randomColor();
		createWordBurst(String(count), (point, color, strobed, strobeColor) => {
			const star = Star.add(
				point.x,
				point.y,
				color,
...
function startNewYearFinale() {
	// Force finale mode in config
	updateConfig({ autoLaunch: true, finale: true });
	// Launch a big "2026" firework immediately
	const shell = new Shell(crysanthemumShell(4));
	shell.launch(0.5, 0.5);
	setTimeout(() => {
		createWordBurst("2026 元旦快乐", (point, color, strobed, strobeColor) => {
			const star = Star.add(point.x, point.y, COLOR.Gold, Math.random() * 2 * Math.PI, 0.2, 3000, 0, -0.05, 3);
			star.strobe = true;
		}, stageW * 0.5, stageH * 0.4);
	}, 1500);
}
